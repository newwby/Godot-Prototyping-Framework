...
func _do_test():
	...
	var test_structure = {
		ConditionalAnd.new(true): [true, true, false, true, true, true], # test_subject_0
		ConditionalAnd.new(false): [true, false, false, false, true, true], # test_subject_1
		ConditionalAnd.new(true, true): [true, true, false, true, true, true], # test_subject_2 (includes base value)
		ConditionalAnd.new(false, true): [false, false, false, false, false, false], # test_subject_3 (includes base value)
		}
	...
	for test_conditional in test_structure.keys():
		if not test_conditional is ConditionalAnd\
		or typeof(test_structure[test_conditional]) != TYPE_ARRAY:
			GlobalLog.trace(self, "test setup error on test for ConditionalAnd")
			continue
		else:
